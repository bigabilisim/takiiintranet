...
class LeaveApprovalMailer
{
    public function send(array $entry): array
    {
        $toEmail = $this->cleanHeader((string) ($entry['to_email'] ?? ''));
        $subject = $this->cleanHeader((string) ($entry['subject'] ?? ''));
        $text = (string) ($entry['body'] ?? '');
        $html = trim((string) ($entry['body_html'] ?? ''));

        if (!filter_var($toEmail, FILTER_VALIDATE_EMAIL)) {
            return ['ok' => false, 'status' => 'invalid_recipient', 'transport' => 'none', 'error' => 'invalid_recipient'];
        }

        $transport = strtolower((string) (getenv('LEAVE_APPROVAL_MAIL_TRANSPORT') ?: getenv('MAIL_TRANSPORT') ?: 'outbox'));
        $transport = in_array($transport, ['native', 'smtp', 'sendmail', 'outbox'], true) ? $transport : 'outbox';

        if ($transport === 'outbox') {
            return ['ok' => true, 'status' => 'queued', 'transport' => 'outbox', 'error' => ''];
        }

        if ($transport === 'sendmail') {
            $result = $this->sendSendmail($toEmail, $subject, $text, $html);

            return $result['ok']
                ? ['ok' => true, 'status' => 'sent', 'transport' => 'sendmail', 'error' => '']
                : ['ok' => false, 'status' => 'queued', 'transport' => 'outbox', 'error' => $this->safeError((string) ($result['error'] ?? 'sendmail_failed'))];
        }

        if ($transport === 'smtp') {
            $result = $this->sendSmtp($toEmail, $subject, $text, $html);

            return $result['ok']
                ? ['ok' => true, 'status' => 'sent', 'transport' => 'smtp', 'error' => '']
                : ['ok' => false, 'status' => 'queued', 'transport' => 'outbox', 'error' => $this->safeError((string) ($result['error'] ?? 'smtp_failed'))];
        }

        $result = $this->sendNative($toEmail, $subject, $text, $html);

        return $result['ok']
            ? ['ok' => true, 'status' => 'sent', 'transport' => (string) ($result['transport'] ?? 'native'), 'error' => '']
            : ['ok' => false, 'status' => 'queued', 'transport' => 'outbox', 'error' => $this->safeError((string) ($result['error'] ?? 'mail_failed'))];
    }

    private function sendNative(string $toEmail, string $subject, string $text, string $html = ''): array
    {
        ...
        $sendmailResult = $this->sendSendmail($toEmail, $subject, $text, $html);
    ...
    private function sendSendmail(string $toEmail, string $subject, string $text, string $html = ''): array
    {
        ...
        fwrite($pipes[0], $this->messagePayload($toEmail, $subject, $text, false, $html));
        fclose($pipes[0]);
    ...
    private function sendSmtp(string $toEmail, string $subject, string $text, string $html = ''): array
    {
        ...
        fwrite($socket, $this->messagePayload($toEmail, $subject, $text, true, $html) . "\r\n.\r\n");
    ...
    private function messagePayload(string $toEmail, string $subject, string $text, bool $dotStuff, string $html = ''): string
    {
        $fromAddress = $this->fromAddress();
        $fromName = $this->fromName();
