...
try {
    ...
    $antalyaOne = createLocationProfile($profiles, [
        'new_email' => 'antalya.one@example.test',
        'first_name' => 'Antalya',
        'last_name' => 'One',
        'department' => 'RD ofis',
    ...
    $antalyaTwo = createLocationProfile($profiles, [
        'new_email' => 'antalya.two@example.test',
        'first_name' => 'Antalya',
        'last_name' => 'Two',
    ...
    $bursa = createLocationProfile($profiles, [
        'new_email' => 'bursa.one@example.test',
        'first_name' => 'Bursa',
        'last_name' => 'One',
    ...
    $antalyaAssistant = createLocationProfile($profiles, [
        'new_email' => 'antalya.hr.assistant@example.test',
        'first_name' => 'Antalya',
    ...
    $bursaAssistant = createLocationProfile($profiles, [
        'new_email' => 'bursa.hr.assistant@example.test',
        'first_name' => 'Bursa',
        'last_name' => 'HR Assistant',
        'department' => 'IK Bursa',
    ...
    $legacyAssistant = createLocationProfile($profiles, [
        'new_email' => 'legacy.hr.assistant@example.test',
        'first_name' => 'Legacy',
        'last_name' => 'HR Assistant',
        'department' => 'IK Antalya',
        'location' => LocationScope::ANTALYA,
    ...
    $conflictingAssistant = $profiles->createProfile([
        'new_email' => 'conflicting.hr.assistant@example.test',
    ...
    $antalyaRecipients = array_column($messages->recipients('antalya.one@example.test'), 'email');
    locationAssert(in_array('antalya.two@example.test', $antalyaRecipients, true), 'Same-location message recipient is missing.');
    locationAssert(!in_array('bursa.one@example.test', $antalyaRecipients, true), 'Cross-location message recipient is visible.');
    locationAssert(empty($messages->send($antalyaOne, [
        'to_email' => 'bursa.one@example.test',
        'subject' => 'Blocked cross-location message',
        'body' => 'This must not be sent.',
    ])['ok']), 'Cross-location message was accepted.');
    locationAssert(!empty($messages->send($antalyaOne, [
        'to_email' => 'antalya.two@example.test',
    ...
    $access->setUserPermissions('antalya.two@example.test', [
        'admin.company.manage',
        'module.messages.access',
        'messaging.send',
    ]);
    $expandedDirectory = $access->usersByIdentity();
    $expandedAntalyaAssistant = $expandedDirectory['antalya.hr.assistant@example.test'] ?? [];
    $expandedBursaAssistant = $expandedDirectory['bursa.hr.assistant@example.test'] ?? [];
    ...
    locationAssert(LocationScope::hasGlobalVisibility($expandedDirectory['antalya.two@example.test'] ?? []), 'Delegated admin did not receive global location visibility.');
    $expandedMessages = new MessageStore($expandedDirectory, $stateStore);
    $delegatedAdminRecipients = array_column($expandedMessages->recipients('antalya.two@example.test'), 'email');
    locationAssert(in_array('bursa.one@example.test', $delegatedAdminRecipients, true), 'Delegated admin cannot see both message locations.');
    locationAssert($access->setDepartmentPolicy('RD ofis', [
        'manager_approval_count' => 1,
        'manager_1_email' => 'bursa.one@example.test',
        'hr_email' => 'hr@example.test',
    ]), 'Cross-location approval policy fixture could not be saved.');
    locationAssert($access->setDepartmentPolicy('Prod ofis', [
        'manager_approval_count' => 1,
        'manager_1_email' => 'bursa.one@example.test',
        'hr_email' => 'hr@example.test',
    ]), 'Bursa approval policy fixture could not be saved.');
    $shifts = new ShiftStore($profiles, $stateStore);
    $antalyaShiftPersonnel = array_column($shifts->personnel($antalyaOne), 'email');
    locationAssert(in_array('antalya.two@example.test', $antalyaShiftPersonnel, true), 'Same-location shift personnel is missing.');
    locationAssert(!in_array('bursa.one@example.test', $antalyaShiftPersonnel, true), 'Cross-location shift personnel is visible.');
    locationAssert(empty($shifts->assignToProfiles('', ['bursa.one@example.test'], false, $antalyaOne)['ok']), 'Cross-location shift assignment was accepted.');
    $hrShiftPersonnel = array_column($shifts->personnel($hr), 'email');
    locationAssert(in_array('antalya.one@example.test', $hrShiftPersonnel, true) && in_array('bursa.one@example.test', $hrShiftPersonnel, true), 'HR cannot see both locations in shift planning.');
    ...
    foreach ([$antalyaOne, $bursa] as $leaveUser) {
        ...
        if (($leaveUser['email'] ?? '') === 'antalya.one@example.test') {
    ...
    foreach ($leave->all() as $leaveRequest) {
        if (($leaveRequest['requester_email'] ?? '') === 'antalya.one@example.test') {
            $antalyaLeaveRequest = $leaveRequest;
        }

        if (($leaveRequest['requester_email'] ?? '') === 'bursa.one@example.test') {
    ...
    locationAssert(in_array('antalya.hr.assistant@example.test', $antalyaSignatureRecipients, true), 'Antalya HR assistant is missing from Antalya signature reminders.');
    locationAssert(!in_array('bursa.hr.assistant@example.test', $antalyaSignatureRecipients, true), 'Bursa HR assistant received an Antalya signature reminder.');
    locationAssert(in_array('bursa.hr.assistant@example.test', $bursaSignatureRecipients, true), 'Bursa HR assistant is missing from Bursa signature reminders.');
    locationAssert(!in_array('antalya.hr.assistant@example.test', $bursaSignatureRecipients, true), 'Antalya HR assistant received a Bursa signature reminder.');
    ...
    foreach ($leaveData['requests'] as &$regionalHrRequest) {
        ...
        if (!in_array($requesterEmail, ['antalya.one@example.test', 'bursa.one@example.test'], true)) {
            continue;
        }

        $regionalHrRequest['status'] = 'waiting_hr';
        $regionalHrRequest['calendar_state'] = 'pending';
        $regionalHrRequest['approval_policy']['hr_email'] = 'hr@example.test';
        ...
        $regionalHrRequest['approvals']['hr']['assignee'] = 'hr@example.test';
    ...
    locationAssert($hrAuth->attempt('hr@example.test', 'location-test-123'), 'HR manager login failed.');
    $hrPendingIds = array_column($leave->pendingApprovalsFor($hrAuth), 'id');
    locationAssert(in_array($regionalHrRequestIds['antalya.one@example.test'], $hrPendingIds, true), 'HR manager cannot see the Antalya HR-stage request.');
    locationAssert(in_array($regionalHrRequestIds['bursa.one@example.test'], $hrPendingIds, true), 'HR manager cannot see the Bursa HR-stage request.');
    $hrAuth->logout();

    $antalyaAssistantAuth = new Auth($profiles, $access);
    locationAssert($antalyaAssistantAuth->attempt('antalya.hr.assistant@example.test', 'location-test-123'), 'Antalya HR assistant login failed.');
    $antalyaPendingIds = array_column($leave->pendingApprovalsFor($antalyaAssistantAuth), 'id');
    locationAssert(in_array($regionalHrRequestIds['antalya.one@example.test'], $antalyaPendingIds, true), 'Antalya HR assistant cannot see the Antalya HR-stage request.');
    locationAssert(!in_array($regionalHrRequestIds['bursa.one@example.test'], $antalyaPendingIds, true), 'Antalya HR assistant can see the Bursa HR-stage request.');
    ...
    $blockedBursaApproval = $leave->advanceByPlatform(
        $regionalHrRequestIds['bursa.one@example.test'],
    ...
    $approvedAntalya = $leave->advanceByPlatform(
        $regionalHrRequestIds['antalya.one@example.test'],
        $antalyaAssistantAuth->user() ?? [],
        $antalyaAssistantAuth,
        'approve'
    );
    locationAssert(!empty($approvedAntalya['ok']), 'Antalya HR assistant could not approve an Antalya request.');
    $approvedAntalyaRequest = $leave->findById($regionalHrRequestIds['antalya.one@example.test']);
    ...
    locationAssert($bursaAssistantAuth->attempt('bursa.hr.assistant@example.test', 'location-test-123'), 'Bursa HR assistant login failed.');
    $bursaPendingIds = array_column($leave->pendingApprovalsFor($bursaAssistantAuth), 'id');
    locationAssert(in_array($regionalHrRequestIds['bursa.one@example.test'], $bursaPendingIds, true), 'Bursa HR assistant cannot see the Bursa HR-stage request.');
    locationAssert(!in_array($regionalHrRequestIds['antalya.one@example.test'], $bursaPendingIds, true), 'Bursa HR assistant can see the Antalya HR-stage request.');
    ...
    $approvedBursaRequest = $leave->findById($regionalHrRequestIds['bursa.one@example.test']);
