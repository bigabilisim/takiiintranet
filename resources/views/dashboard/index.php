...
<section class="dashboard-grid dashboard-secondary">
    <div class="work-queue">
        <div class="section-title">
            <h2><?= htmlspecialchars($t('dashboard.queue'), ENT_QUOTES, 'UTF-8') ?></h2>
        </div>
        <div class="queue-list">
            <?php foreach ($workQueue as $item): ?>
                <div class="queue-row">
                    <span><?= htmlspecialchars($t($item['type_key']), ENT_QUOTES, 'UTF-8') ?></span>
                    <strong><?= htmlspecialchars((string) $item['count'], ENT_QUOTES, 'UTF-8') ?></strong>
                    <small><?= htmlspecialchars($t($item['status_key']), ENT_QUOTES, 'UTF-8') ?></small>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="module-area">
        <div class="section-title">
            <h2><?= htmlspecialchars($t('dashboard.modules'), ENT_QUOTES, 'UTF-8') ?></h2>
        </div>
        <div class="module-grid">
            <?php foreach ($visibleModules as $module): ?>
                <a class="module-card" href="/module/<?= htmlspecialchars($module['slug'], ENT_QUOTES, 'UTF-8') ?>" style="--accent: <?= htmlspecialchars($module['accent'], ENT_QUOTES, 'UTF-8') ?>">
                    <span class="module-code"><?= htmlspecialchars($module['code'], ENT_QUOTES, 'UTF-8') ?></span>
                    <h3><?= htmlspecialchars($t($module['title_key']), ENT_QUOTES, 'UTF-8') ?></h3>
                    <p><?= htmlspecialchars($t($module['summary_key']), ENT_QUOTES, 'UTF-8') ?></p>
                    <div class="metric-row">
                        <?php foreach ($module['metrics'] as $metric): ?>
                            <span>
                                <strong><?= htmlspecialchars($metric['value'], ENT_QUOTES, 'UTF-8') ?></strong>
                                <?= htmlspecialchars($t($metric['label_key']), ENT_QUOTES, 'UTF-8') ?>
                            </span>
                        <?php endforeach; ?>
                    </div>
                </a>
